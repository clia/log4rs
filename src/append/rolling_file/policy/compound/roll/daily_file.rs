...
pub struct DailyFileRollerConfig {
    pattern: String,
    base: Option<u32>,
    count: u32,
}
...
///
/// A `DailyFileRoller` is configured with a filename pattern, a base index,
...
impl DailyFileRoller {
    /// Returns a new builder for the `DailyFileRoller`.
    pub fn builder() -> DailyFileRollerBuilder {
        DailyFileRollerBuilder { base: 0 }
    }
}
...
impl Roll for DailyFileRoller {
    fn roll(&self, file: &Path) -> Result<(), Box<dyn Error + Sync + Send>> {
        ...
        let dst_0 = self.pattern.replace("{}", &format!("{}", Local::today().pred().format("%Y-%m-%d")));
...
/// A builder for the `DailyFileRoller`.
pub struct DailyFileRollerBuilder {
    base: u32,
}

impl DailyFileRollerBuilder {
    /// Sets the base index for archived log files.
    ///
    /// Defaults to 0.
    pub fn base(mut self, base: u32) -> DailyFileRollerBuilder {
        self.base = base;
        self
    }

    /// Constructs a new `DailyFileRoller`.
    ///
...
/// A deserializer for the `DailyFileRoller`.
///
/// # Configuration
///
/// ```yaml
...
/// ```
#[cfg(feature = "file")]
pub struct DailyFileRollerDeserializer;
...
mod test {
    ...
    fn rotation() {
        ...
        let roller = DailyFileRoller::builder()
